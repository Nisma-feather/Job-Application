...
const JobCard = ({ item }) => {
  ...
  const fetchBookMarks = async () => {
    try {
      ...
      const bookmarks = bookmarkSnap.docs.map(
        (bookmark) => bookmark.data().jobId
      );
  ...
  const fetchReviews = async () => {
    ...
    setReviewAvg(snapData.data()?.reviewAvg || 0);
  };
  ...
  const handletoggleBookmark = async (jobId) => {
    try {
      ...
      if (!bookmarkSnap.empty) {
        ...
        setBookmarkJobs(bookmarkJobs.filter((id) => id !== jobId));
      } else {
  ...
  const isExpired =
    new Date(item.expiryDate?.toDate?.() || item.expiryDate) < new Date();

  return (
    <Pressable
      onPress={() => navigation.navigate("Job Details", { currentJob: item })}
      disabled={item?.status === "Closed" || isExpired} // Disable card press if Closed or Expired
      ...
      <View style={styles.jobItem}>
        <View style={{ flexDirection: "row", justifyContent: "space-between"}}>
            ...
            <View style={{ justifyContent: "space-between" }}>
              ...
              {item?.status === "Closed" ? (
                <View style={styles.statusContainer}>
                  <View
                    style={[styles.statusDot, { backgroundColor: "#d32f2f" }]}
                  />
          ...
          >
            <Ionicons
              name={
                bookmarkJobs.includes(item.id) ? "bookmark" : "bookmark-outline"
              }
              color="#4B9CD3"
              size={24}
            />
...
const styles = StyleSheet.create({
  ...
  statusContainer: {
    flexDirection: "row",
    alignItems: "center", // vertically centers dot and text
    marginTop: 4,
    flexShrink: 1, // prevents text from overflowing
    gap: 6, // space between dot and label
  },
  statusDot: {
    width: 10, // slightly larger circle
    height: 10,
    borderRadius: 5,
    flexShrink: 0,
  },
  statusLabel: {
    ...
    color: "#555",
    fontWeight: "600",
    flexShrink: 1, // text shrinks if necessary
  },
});
